...
export default {
  initialize (newPrefix) {
    ...
    filters = JSON.parse(sessionStorage.getItem(`${prefix}/filters`)) || {
      whitelist: [],
      whiteregex: /.*/,
      blacklist: [],
      blackregex: null
  ...
  clearAll () {
    ...
    sessionStorage.setItem(`${prefix}/servers`, servers)
    sessionStorage.setItem(`${prefix}/tracks`, tracks)
    sessionStorage.setItem(`${prefix}/variants`, variants)
    sessionStorage.setItem(`${prefix}/features`, features)
    sessionStorage.setItem(`${prefix}/tools`, tools)
    sessionStorage.setItem(`${prefix}/filters`, filters)
  },
  servers: {
  ...
  filters: {
    get: () => _.cloneDeep(filters),
    set: (updates) => {
      _.merge(filters, updates)
