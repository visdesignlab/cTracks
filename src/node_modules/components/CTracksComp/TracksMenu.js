  ...
  makeSubscriptions() {
    return [
      hamradio.subscribe(
        'track/add',
        (name, row) => {
          if (!this.state.editTrack) {
            console.log(row)
            this.editTrack(row, Object.values(row.tilesets)[0])
          }
        }
      ),
      hamradio.subscribe(
  ...
  trackOverview = () => {
    return (
      <div>
        ...
        <TrackAdd
          servers={this.props.servers}
          row={Math.max(0, ...Object.values(this.props.tracks).map(row => row.row))+1}
        />
      </div>
		)
  }
      ...
      <div>
        ...
        <ul>
          {this.state.editTrack.options.constIndicators
            ? this.state.editTrack.options.constIndicators.map((ruler, index) => (<li key={index}>{`${ruler.label}: ${ruler.value}`}<IconButton onClick={() => this.removeConstant(index)} icon="minus"/></li>))
            : null
          }
          <li>
            <label>Label:</label>
            <input type="text" name="constant-label" style={{maxWidth:80}}
              value={this.state.editConstant.label} onChange={e => this.setState({editConstant: {...this.state.editConstant, label: e.target.value}})}
            />
            <label>Value:</label>
            <input type="number" name="constant-value" style={{maxWidth:80}}
              value={this.state.editConstant.value} onChange={e => this.setState({editConstant: {...this.state.editConstant, value: e.target.value}})}
            />
            <IconButton onClick={() => this.addConstant()} icon="plus"/>
          </li>
        </ul>
      </div>
    )
