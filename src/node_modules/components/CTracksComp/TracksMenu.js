...
class TracksMenu extends React.Component {
  ...
  makeSubscriptions = () => {
    return [
      hamradio.subscribe(
        'track/add',
        (name, data) => {
          if (this.state.tracks.findIndex(track => track.uid === data.uid) === -1) {
            this.setState({
              tracks: [
                ...this.state.tracks,
                {
                  ...data,
                  global: name.search('global') !== -1,
                  focus: name.search('focus') !== -1
                }
              ]
            })
          }
  ...
  createCheckBox (track, index) {
    var KeyGlobal = track.label + '_globalView';
    var KeyFocus = track.label + '_focusView';
