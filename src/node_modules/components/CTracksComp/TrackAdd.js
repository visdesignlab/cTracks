...
class TrackAdd extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      tracks: [],
      track: {value: newTrack(props.initOrder), label:'*New Track*'},
      name: ''
    }
  }
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    ...
    return {
      ...
      tracks: [{value: newTrack(nextProps.initOrder), label:'*New Track*'}].concat(...Object.values(nextProps.tracks).map(track => {
        return {
          value: track,
          label: track.label || `Track ${track.order+1}`
        }
      }))
    }
  }
  ...
  makeSubscriptions() {
    return [
      hamradio.subscribe(
        ...
        (name, data) => {
          this.setState({
            track: {value: newTrack(this.props.initOrder), label:'*New Track*'},
            tileset: {value: null, label:'', server: {}, tileset: {}},
            name: ''
          })
        }
